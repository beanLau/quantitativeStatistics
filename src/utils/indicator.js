export default {
    ...
    ema: function (lastEma, closePrice, units) {
        return (lastEma * (units - 1) + closePrice * 2) / (units + 1);
    },

    dea: function (lastDea, curDiff) {
    ...
    macd: function (ticks) {
        ...
        for (var i = 0; i < ticks.length; i++) {
            ...
            if (i == 0) {
                ...
                ema12.push(this.ema(ema12[i - 1], c, 12));
                ema26.push(this.ema(ema26[i - 1], c, 26));
            }
            diffs.push(ema12[i] - ema26[i]);
            if (i != 0) {
                deas.push(this.dea(deas[i - 1], diffs[i]));
            }
    ...
    getMaxHighAndMinLow: function (ticks) {
    ...
    kdj: function (ticks) {
        ...
        for (var i = 0; i < ticks.length; i++) {
            ...
            maxAndMin = this.getMaxHighAndMinLow(nineDaysTicks);
            max = maxAndMin[0];
            min = maxAndMin[1];
            if (max == min) {
                rsvs.push(0);
